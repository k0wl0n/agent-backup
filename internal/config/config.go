...
func (c *Config) Validate() error {
	...
	if c.Agent.Type != "" && c.Agent.Type != "host" && c.Agent.Type != "managed-runner" && c.Agent.Type != "headless" && c.Agent.Type != "serverless" {
		errs = append(errs, fmt.Sprintf("agent.type %q is invalid; must be \"host\", \"headless\", \"managed-runner\", or \"serverless\"", c.Agent.Type))
	}
